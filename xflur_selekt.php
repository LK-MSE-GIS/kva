...
if (($mitarb_id =='0') OR ($mitarb_id ==''))
  ...
  if ($fselekt == '50')$query=$sel_r["query"]." AND alkis_feld_mitid='$mitarb_id'";
  if ($fselekt == '52')$query=$sel_r["query"]." AND alkis_feld_mitid='$mitarb_id'";
  if ($fselekt == '51')$query=$sel_r["query"]." AND alkis_felddb_mitid='$mitarb_id'";
  if ($fselekt == '55')$query=$sel_r["query"]." AND alkis_albalk_mitid='$mitarb_id'";
  if ($fselekt == '56')$query=$sel_r["query"]." AND alkis_albalk_mitid='$mitarb_id'";
  if ($fselekt == '102') $query=$sel_r["query"]." AND alkis_akt1_mitid='$mitarb_id'";
  if ($fselekt == '103')$query=$sel_r["query"]." AND alkis_st2_mitid='$mitarb_id'";
  if ($fselekt == '104')$query=$sel_r["query"]." AND alkis_st3_mitid='$mitarb_id'";
  }



 if ($bov_id >'0') $query=$sel_r["query"]." AND bov='$bov_id'";
$query=$query." ORDER BY gemkg_id,flur_id";
...
<tr style=\"font-family:Arial; font-size: 12pt; font-weight: bold\">
 <td colspan=\"6\" align=\"center\">$sel_r[tabtext]<br></td>
</tr>
          ...
          if ($fselekt == 20)
           {
            ...
            <td>&nbsp;$r[gemkg_id]&nbsp;</td>
            <td>&nbsp;$r[flur_id]&nbsp;</td>
            <td>&nbsp;$bovr[ausfstelle]-$bovr[verfnr]($bovr[Name])</td>
            <td>&nbsp;";
            if ($bovr["busy"]=='0') echo "abgeschlossen";
            if ($bovr["busy"]=='1') echo "in Arbeit bis $bovr[abschl]";
            echo "&nbsp;</td>
                ...
                if ($sel_r["feld3_db"] =='1')
                 {
                  ...
                  $mresult=mysqli_query($db_link,$mquery);
                  $rm=mysqli_fetch_array($mresult);
                  echo "$rm[name]";
                 }
                }
               else echo "$r[$feld3]";
                ...
                if ($sel_r["feld4_db"] =='1')
                 {
                  ...
                  $m2result=mysqli_query($db_link,$m2query);
                  $rm2=mysqli_fetch_array($m2result);
                  echo "$rm2[name]";
                 }
                }
               else
