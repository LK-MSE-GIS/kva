...
echo "<div class=\"ausgabe_bereich\" style=\"font-family:Arial; font-size: 10pt; font-weight: bold\">";
if ($alt == 'no') nav_aendern($id,$db_link,$page,$status);
  else nav_aendern_alt($id,$db_link,$page,$status);
 echo "</div>";
echo "<div class=\"ausgabe_bereich\" style=\"font-family:Arial; font-size: 10pt; font-weight: bold\">";
echo "<table border=\"0\">
<tr bgcolor=\"#EFA036\">
...
$r_count=0;
  ...
  if ($r["gemark_id_1"] > 0)
   {
    echo $r["gemark_id_1"]," (",get_gemark_name($r["gemark_id_1"],$db_link),") ";
    if ($plus == $r["gemark_id_1"])
      {
        $vorschlag=get_last_riss($r["gemark_id_1"],$db_link)+1;
      ...
      {
       if (strpos($abteilung,"ris") > 0) echo " <a href= \"xant_nachweise.php?id=$id&status=$status&page=$page&alt=$alt&plus=$r[gemark_id_1]\"><img src=\"images/buttons/plus2.png\" border=\"0\" alt =\"Rissnummer hinzufügen\" width=15></a><br>";
      }
    $riss_query="SELECT riss_id FROM risse2antrag WHERE antrag_id ='$id' AND gemark_id = '$r[gemark_id_1]'";
    $riss_result=mysqli_query($db_link,$riss_query);    
    while($r_riss=mysqli_fetch_array($riss_result))
    {
  ...
  if ($r["gemark_id_2"] > 0 AND  $r["gemark_id_2"] != $r["gemark_id_1"])
   {
    echo $r["gemark_id_2"]," (",get_gemark_name($r["gemark_id_2"],$db_link),") ";
    if ($plus == $r["gemark_id_2"])
      {
        $vorschlag=get_last_riss($r["gemark_id_2"],$db_link)+1;
        echo"<form action=\"ant_riss.php\" method=\"post\" target=\"\">
        <input type=hidden name=\"id\" value=\"$id\">
        <input type=hidden name=\"status\" value=\"$status\">
        <input type=hidden name=\"page\" value=\"$page\">
      ...
      {
       if (strpos($abteilung,"ris") > 0) echo " <a href= \"xant_nachweise.php?id=$id&status=$status&page=$page&alt=$alt&plus=$r[gemark_id_2]\"><img src=\"images/buttons/plus2.png\" border=\"0\" alt =\"Rissnummer hinzufügen\" width=15></a><br>";
      }
  ...
  if ($r["gemark_id_3"] > 0 AND  $r["gemark_id_3"] != $r["gemark_id_1"] AND  $r["gemark_id_3"] != $r["gemark_id_2"])
   {
    echo $r["gemark_id_3"]," (",get_gemark_name($r["gemark_id_3"],$db_link),") ";
    if ($plus == $r["gemark_id_3"])
      {
        $vorschlag=get_last_riss($r["gemark_id_3"],$db_link)+1;
        echo"<form action=\"ant_riss.php\" method=\"post\" target=\"\">
        <input type=hidden name=\"id\" value=\"$id\">
        <input type=hidden name=\"status\" value=\"$status\">
        <input type=hidden name=\"page\" value=\"$page\">
      ...
      {
       if (strpos($abteilung,"ris") > 0) echo " <a href= \"xant_nachweise.php?id=$id&status=$status&page=$page&alt=$alt&plus=$r[gemark_id_3]\"><img src=\"images/buttons/plus2.png\" border=\"0\" alt =\"Rissnummer hinzufügen\" width=15></a><br>";
      }
